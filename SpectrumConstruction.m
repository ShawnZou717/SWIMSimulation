...
if strcmp(ocean_type, 'PM')
    ...
    FD = exp(-5 / 4 * (k_peak ./ K) .^ 2) * (0.0081 / 2) .* K .^ (-4) .* 4 ...
        .* cos((WindDirection - ObservingAngle) - fine_transmit) .^ 4 ./ (3 * pi);%PM谱波高谱变量
...
elseif strcmp(ocean_type, 'JONS')
    ...
    fine_transmit = delta_phi + atan((ky ./ kx));
    FD = alfa / 2 .* K .^ (-4) .* exp(-5 / 4 .* (k_peak ./ K) .^ 2) .* gama...
        .^ exp(-((K ./ k_peak) .^ 0.5 - 1) .^ 2 ./ (2 .* deltas1 .^ 2)) .* 4 .* cos((WindDirection - ObservingAngle) - fine_transmit) .^ 4 ./ (3 * pi);
