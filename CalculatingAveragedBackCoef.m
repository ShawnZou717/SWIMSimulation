...
% Ku band Fresnel reflection ratio of 20^oC sea water
lo = 0.61;

WindDirection = nan;
for index = 1 : length(OceanWaveType)
    if strcmp(OceanWaveType(index), "EL") || strcmp(OceanWaveType(index), "PM") || strcmp(OceanWaveType(index), "JSON")
...
% There must be a wind wave among the ocean type.
if isnan(WindDirection)
    error('There is no wind wave in the ocean type.')
end

% Looking direction relative to the wind direction.
